...
export default function WeatherWidget() {
    ...
    const handleSearch = async(e: FormEvent<HTMLFormElement>) => {
        ...
        try{
            const response = await fetch(
                `http://api.weatherapi.com/v1/current.json?key=${process.env.NEXT_PUBLIC_WEATEHER_API_KEY}&q=${trimmedLocation}`
            );
            if(!response.ok) {
    ...
    function getWeatherMessage (description: string): string{
        switch (description.toLocaleLowerCase()){
            ...
            case "partly cloudy":
                return "Expect some cloudy and sunshine.";
            case "cloudy":
                return "It's cloudy today.";
            case "overcast":
                return "The sky is overcast.";
            case "rain":
                return "Don't forget to bring an umbrella! It's raining.";
            case "thunderstorm":
